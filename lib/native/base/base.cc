...
{
    // Without the forced alignment, the compiler is forced to use slow libatomic-based calls
    // on x86 32-bit targets, because int64_t is 4-byte aligned on these platforms.
    // In addition to being slow, it fails to work in Koffi because the atomic_load() function
    // is not available.
    typedef __attribute__((aligned(8))) int64_t aligned_int64_t;

    static aligned_int64_t *addr = []() -> aligned_int64_t * {
