...
const IO = {
    ...
    xDeviceCharacteristics: function (pFile) {
        return capi.SQLITE_IOCAP_ATOMIC | capi.SQLITE_IOCAP_SUBPAGE_READ;
    },
    ...
    xWrite: function (pFile, pSrc, n, offset64) {
        ...
        if (offset + n > file.u8.byteLength) {
            try {
...
const VFS = {
    ...
    xOpen: function (pVfs, zName, pFile, flags, pOutFlags) {
        ...
        if (file == null) {
            ...
            let sab = new SharedArrayBuffer(0, { maxByteLength: max_file_size });
...
function init(sqlite3, max_size) {
...
function write(name, sab) {
    if (max_file_size == null)
        throw new Error('SABFS is not initialized');
...
function shrink(name) {
    ...
    if (file == null)
        throw new Error(`Cannot shrink '${name}' file`);

    if (file.u8.byteLength < file.buf.byteLength) {
        let sab = new SharedArrayBuffer(file.u8.byteLength, { maxByteLength: max_file_size });
