...
function Workbook(options = {}) {
    ...
    {
        let core = xmls['docProps/core.xml'];
        let app = xmls['docProps/app.xml'];

        let date = options.date ?? new Date;
        let mtime = (new Date(date)).toISOString().replace(/\.[0-9]{3}/, '');

        core.querySelector('created').textContent = mtime;
        core.querySelector('modified').textContent = mtime;

        if (options.app != null) {
            let name = options.app + (options.version != null ? ' ' + options.version : '');

            core.querySelector('creator').textContent = name;
            app.querySelector('Application').textContent += ' / ' + name;
        }

        if (options.version != null) {
            let version = options.version.split(/[^0-9]/).slice(0, 2).join('.');

            if (!version.match(/^[0-9]+\.[0-9]+$/))
                throw new Error('Malformed AppVersion number');

            app.querySelector('AppVersion').textContent = version;
            app.querySelector('AppVersion').textContent = version;
        }
    }
    ...
    this.createSheet = function (name = null) {
    ...
    this.save = async function (options = {}) {
        ...
        // Drop unwanted XML files
        delete xmls['xl/calcChain.xml'];
        ...
        let core = xmls['docProps/core.xml'];
        let types = xmls['[Content_Types].xml'];
        let workbook = xmls['xl/workbook.xml'];
        let rels = xmls['xl/_rels/workbook.xml.rels'];

        // Update modification time
        if (core != null) {
            let modified = core.querySelector('modified');

            if (modified != null) {
                let mtime = (new Date(date)).toISOString().replace(/\.[0-9]{3}/, '');
                modified.textContent = mtime;
            }
        }
        ...
        // Update sheets and workbook rels
        {
            ...
            let types_el = types.documentElement;
            ...
            for (let node of types_el.children) {
                if (node.tagName == 'Override') {
                    let type = node.getAttribute('ContentType');
                    let part = node.getAttribute('PartName');

                    if (part.startsWith('/'))
                        part = part.substr(1);

                    if (type == constants.WORKSHEET_TYPE)
                        continue;
                    if (!Object.hasOwn(xmls, part))
                        continue;
            ...
            for (let [name, sheet] of Object.entries(sheets)) {
                let type = types.createElementNS(constants.CONTENT_TYPES_NS, 'Override');
                let rel = rels.createElementNS(constants.RELS_NS, 'Relationship')
                let node = sheet_map.get(name) ?? workbook.createElementNS(constants.WORKSHEET_NS, 'sheet');
                ...
                type.setAttribute('PartName', '/xl/' + filename);
                type.setAttribute('ContentType', constants.WORKSHEET_TYPE);

                rel.setAttribute('Type', constants.REL_WORKSHEET_NS);
                ...
                if (!node.hasAttribute('state'))
                    node.setAttribute('state', 'visible');
                node.setAttribute('sheetId', sid);
                node.setAttributeNS(constants.REL_NS, 'id', rid);

                new_types.push(type);
                new_rels.push(rel);
                new_sheets.push(node);
            }

            types_el.replaceChildren(...new_types);
            rels_el.replaceChildren(...new_rels);
            sheets_el.replaceChildren(...new_sheets);
        }

        // Force some workbook attributes
        {
            let calcpr = workbook.querySelector('calcPr');

            if (calcpr == null) {
                calcpr = workbook.createElementNS(constants.WORKSHEET_NS, 'calcPr');
        ...
        // Export XML files
        {
            ...
            for (let [name, doc] of Object.entries(xmls)) {
                let str = serializer.serializeToString(doc.documentElement).replace(/\r?\n/g, '\r\n');
    ...
    function cleanSheet(sheet, formulas) {
        ...
        for (let node of data.querySelectorAll('c')) {
            ...
            if (ref) {
                ...
                rect.left = Math.min(rect.left, column);
                rect.right = Math.max(rect.right, column);
                rect.top = Math.min(rect.top, row);
                rect.bottom = Math.max(rect.bottom, row);
            }
        }
        ...
        // Update dimensions
        {
            let dims = sheet.querySelector('dimension');

            if (rect.left == Number.MAX_SAFE_INTEGER)
                rect.left = 0;
            if (rect.top == Number.MAX_SAFE_INTEGER)
                rect.top = 0;

            let range = makeRef(rect.left, rect.top) + ':' + makeRef(rect.right, rect.bottom);
            dims.setAttribute('ref', range);
        }

        if (!formulas) {
            // Avoid :has for compatibility (for now)
            let cells = Array.from(data.querySelectorAll('c > f')).map(f => f.parentElement);
            let values = cells.flatMap(cell => Array.from(cell.querySelectorAll('v')));
...
function makeRef(column, row) {
    ...
    for (; column; column = Math.floor((column - 1) / 26)) {
        let value = (column - 1) % 26;
        str = String.fromCharCode(65 + value) + str;
    }
    str += String(row);
