...
const DEFAULT_LOG_TIMEOUT = 6000;
...
const Util = new function () {
    ...
    this.setLocale = function (lang) {
        ...
        initTranslationFunctions(table.messages);
...
function initTranslationFunctions(messages) {
    T.format = (str, ...args) => {
...
const Log = new function() {
    ...
    // Share handlers between bundles
    let handlers = (() => {
        let symbol = Symbol.for('LogHandlers');
    ...
    Object.defineProperties(this, {
        entries: { get: () => entries, enumerable: true }
    });
    ...
    function updateEntry(entry, type, msg, timeout) {
        ...
        if (timeout != null && timeout >= 0)
            entry.timer = setTimeout(() => closeEntry(entry), timeout);
        ...
        for (let i = handlers.length - 1; i >= 0; i--) {
            let func = handlers[i];
            let fallthrough = func(is_new ? 'open' : 'edit', entry);
    ...
    function closeEntry(entry) {
        ...
        for (let i = handlers.length - 1; i >= 0; i--) {
            let func = handlers[i];
            let fallthrough = func('close', entry);
    ...
    this.Entry = function () {
        ...
        this.click = () => self.close();
        this.visible = () => true;

        // Used by progress entries
        this.value = null;
        this.max = null;

        this.debug = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return updateEntry(self, 'debug', msg, timeout); };
        this.info = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return updateEntry(self, 'info', msg, timeout); };
        this.success = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return updateEntry(self, 'success', msg, timeout); };
        this.warning = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return updateEntry(self, 'warning', msg, timeout); };
        this.error = function (msg, timeout = DEFAULT_ERROR_TIMEOUT) { return updateEntry(self, 'error', msg, timeout); };

        this.progress = function (action, value = null, max = null) {
            let entry = updateEntry(self, 'progress', action, null);

            entry.value = value ?? null;
            entry.max = max ?? (value != null ? entry.max : null);
    ...
    this.debug = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return (new self.Entry).debug(msg, timeout); };
    this.info = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return (new self.Entry).info(msg, timeout); };
    this.success = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return (new self.Entry).success(msg, timeout); };
    this.warning = function (msg, timeout = DEFAULT_LOG_TIMEOUT) { return (new self.Entry).warning(msg, timeout); };
    this.error = function (msg, timeout = DEFAULT_ERROR_TIMEOUT) { return (new self.Entry).error(msg, timeout); };
    this.progress = function (action, value = null, max = null) { return (new self.Entry).progress(action, value, max); };

    function defaultHandler(action, entry) {
        if (action == 'close')
            return true;
        ...
        switch (entry.type) {
            case 'debug':
            case 'info':
            case 'success': { console.log(entry.msg); } break;
            case 'warning':
            case 'error': { console.error(entry.msg); } break;
        }

        // Useless (last handler) but does not hurt
        return true;
    };
};
...
const Net = new function() {
    ...
    this.cache = async function (key, url, transform = json => json) {
    ...
    this.loadImage = async function (url, options = {}) {
    ...
    this.readError = async function (text, nice = true) {
        if (typeof text != 'string')
            text = (await text.text()).trim();
...
function LruMap(limit) {
    ...
    this.entries = function*() {
        for (let [key, bucket] of map.entries())
...
class LocalDate {
    ...
    clone() { return new LocalDate(this.year, this.month, this.day); }

    isZero() { return !this.year && !this.month && !this.day; }
    isValid() {
    ...
    isComplete() { return this.isValid() && this.day != null; }
    ...
    valueOf() { return this.toJulianDays; }
    ...
    isoWeekDay() {
        let weekday = 1 + (this.toJulianDays() % 7);
        return weekday;
    }

    diff(other) { return this.toJulianDays() - other.toJulianDays(); }
    ...
    minusMonths(months) { return this.plusMonths(-months); }
    ...
    static parse(str, validate = true) {
        if (str == null)
            return null;
...
class LocalTime {
    ...
    clone() { return new LocalTime(this.hour, this.minute, this.second); }

    isZero() { return !this.hour && !this.minute && !this.second; }
    isValid() {
    ...
    toDaySeconds() { return (this.hour * 3600) + (this.minute * 60) + this.second; }
    valueOf() { return this.toDaySeconds() }

    diff(other) { return this.toDaySeconds() - other.toDaySeconds(); }
...
class FileReference {
    constructor(sha256, name, url = null) {
        this.sha256 = sha256;
        this.name = name;
        this.url = url;
    }
