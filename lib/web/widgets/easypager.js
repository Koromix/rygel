...
function EasyPager() {
    ...
    function renderWidget() {
        ...
        return html`
            <table>
                ${makePageLink(T.previous_page, current_page > 1 ? (current_page - 1) : null, 'special')}
                ${start_page > 1 ?
                ...
                ${makePageLink(T.next_page, current_page < last_page ? (current_page + 1) : null, 'special')}
            </table>
        `;
    }

    function makePageLink(text, page, cls = '') {
        if (page) {
            return html`<td class=${cls}><a href=${url_builder?.call?.(self, page) ?? ''}
                                            @click=${e => handlePageClick(e, page)}>${text}</a></td>`;
        } else {
