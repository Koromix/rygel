...
let prev_main_strings = null;
let prev_main_close = null;
...
function logHandler(action, entry) {
    // Wait a bit to show progress entries to prevent quick actions from showing up
    let wait = (action == 'open' && entry.type == 'progress');
    setTimeout(renderLog, wait ? 300 : 0);
...
function renderLog() {
    ...
    render(Util.mapRange(0, entries.length, idx => {
        let entry = entries[entries.length - idx - 1];
        ...
        return html`
            <div class=${entry.type} @click=${e => entry.click?.()}>
                <span>${msg}</span>
                ${entry.type == 'progress' && entry.value != null ? html`<progress value=${entry.value} max=${entry.max}></progress>` : ''}
            </div>
        `;
    }), el);
}
...
function wrap(func) {
    return async e => {
        ...
        if (target != null) {
            ...
            if (busy.disabled)
                return;
            if (busy.classList.contains('busy'))
                return;
            if (busy.classList.contains('flash'))
                return;
        }
...
function flash(el, text, delay = 4000) {
    if (el.classList.contains('flash'))
        return;

    let content = `'${text.replaceAll("'", "\\'")}'`;

    el.style.setProperty('--text', content);
...
function main(obj = null, options = {}) {
    ...
    if (obj != null) {
        ...
        if (overlay == null && obj.content.strings !== prev_main_strings) {
            if (prev_main_close != null)
                prev_main_close();
            prev_main_close = obj.close;

            if (obj.open != null)
                obj.open();

            main_el.scrollIntoView({
                block: 'start',
                behavior: 'instant'
            });
            if (focus)
            ...
            prev_main_strings = obj.content.strings;
        }
    }
}
...
function tableHeader(key, by, title) {
    ...
    return html`
        <th class="item" @click=${e => { setOrder(key, by); run_func(); }}>
            ${title}
            <div class="arrows">
                <span class=${'up' + (ascending === true ? ' active' : '')}></span>
                <span class=${'down' + (ascending === false ? ' active' : '')}></span>
            </div>
        </th>
    `;
}
