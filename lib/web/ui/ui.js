...
function blockClose(func = null) {
    let handle = Symbol();
    close_handles.set(handle, func);
    return handle;
}

function unblockClose(handle) {
    close_handles.delete(handle);
}

function isCloseBlocked() {
    setTimeout(() => {
        for (let func of close_handles.values()) {
            if (func != null)
                func();
        }
    }, 0);

    return close_handles.size;
}
