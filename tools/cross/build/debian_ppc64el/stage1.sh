...
fakeroot debootstrap \
    --arch=$ARCH \
    --include="linux-image-powerpc64le,openssh-server" \
    $SUITE $TARGET
...
curl -L -o- https://nodejs.org/dist/v24.13.0/node-v24.13.0-linux-ppc64le.tar.xz | tar -xJ -C $TARGET/opt/node --strip-components=1
...
mkdir $SYSROOT/lib64
ln -s ../usr/lib/powerpc64le-linux-gnu/ld64.so.2 $SYSROOT/lib64/ld64.so.2

cp -L $TARGET/vmlinux $DEST/vmlinux
chmod -x $DEST/vmlinux
