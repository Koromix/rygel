...
bool ParserImpl::Parse(const TokenizedFile &file)
{
    ...
    // Remove garbage possibly generated by ParsePrototypes()
    src->lines.RemoveFrom(0);
    src->lines.Append({1, out_program->ir.len});
    ...
    while (pos < tokens.len) {
        switch (tokens[pos].kind) {
            case TokenKind::EndOfLine: {
                src->lines.Append({tokens[pos].line + 1, out_program->ir.len});
                pos++;
            } break;
