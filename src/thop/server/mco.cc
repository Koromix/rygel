...
struct BuildReadableGhmTreeContext {
    ...
    Span<McoReadableGhmNode> out_nodes;
...
static bool ProcessGhmNode(BuildReadableGhmTreeContext &ctx, Size ghm_node_idx)
{
    for (Size i = 0;; i++) {
        ...
        McoReadableGhmNode *out_node = &ctx.out_nodes[ghm_node_idx];
...
static bool BuildReadableGhmTree(Span<const mco_GhmDecisionNode> ghm_nodes,
                                 HeapArray<McoReadableGhmNode> *out_nodes,
                                 Allocator *str_alloc)
{
    if (!ghm_nodes.len)
        return true;
...
bool InitMcoTables(Span<const char *const> table_directories)
{
    ...
    for (const mco_TableIndex &index: mco_table_set.indexes) {
        HeapArray<McoReadableGhmNode> *readable_nodes = mco_cache_set.readable_nodes.SetDefault(&index);
        if (!BuildReadableGhmTree(index.ghm_nodes, readable_nodes, &mco_cache_set.str_alloc))
            return false;
    }
