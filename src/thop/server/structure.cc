    ...
    bool valid = true;
    {
        ...
        while (ini.Next(&prop)) {
            ...
            do {
                ...
                bool inserted = units_set.InsertOrFail(ent.unit);

                if (inserted) {
                    structure.entities.Append(ent);

                    Size *ptr = unit_reference_counts.InsertOrGet(ent.unit, 0);
                    (*ptr)++;
                } else {
            ...
            bool inserted = structures_set.InsertOrFail(structure.name);
