...
static bool ApplySandbox(Span<const char *const> reveals)
{
    ...
    // We need to access the network to send mails and for OIDC calls
    ...
    sb.FilterSyscalls({
        ...
        { "socket/unix", sb_FilterAction::Allow },
        { "socket/inet", sb_FilterAction::Allow },
    ...
    // Embedded static asset?
    {
        ...
        if (path == "/" || ext == "" || ext == ".html") {
            ...
            Span<const char> csp = Fmt(io->Allocator(), "base-uri 'none'; "
                                                        "default-src 'self'; "
                                                        "script-src 'self' 'nonce-%1' 'wasm-unsafe-eval'; "
