...
{
    const char *filename = GetEnv("JUMBLL_DATABASE");
...
static bool DistributeChanges(BackupSet *set, bool checksum)
{
    ...
    bool success = set->db.Transaction([&]() {
        ...
        for (const char *disk_path: config.disk_paths) {
            const char *uuid_filename = Fmt(&temp_alloc, "%1.jumbll", disk_path).ptr;
...
Options:
    ...
    {
        const char *filename = Fmt(&temp_alloc, "%1%/.jumbll", disk_path).ptr;
...
Commands:
    ...
    %!..+init%!0                           Init jumbll database for backups
