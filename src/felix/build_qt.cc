...
const char *Builder::AddQtUiSource(const SourceFileInfo &src)
{
    ...
    if (!header_filename) {
        ...
        const char *text = Fmt(&str_alloc, StdErr->IsVt100(), "Build UI %!..+%1%!0 %!D..@%2%!0", src.filename, src.target->ns).ptr;
...
const char *Builder::AddQtResource(const TargetInfo &target, Span<const char *> qrc_filenames)
{
    ...
    if (!cpp_filename) {
        cpp_filename = Fmt(&str_alloc, "%1%/%2_qrc.cc", gen_directory, target.name).ptr;
...
bool Builder::CompileMocHelper(const SourceFileInfo &src, Span<const char *const> system_directories, uint32_t features)
{
    ...
    if (!moc_filename) {
        ...
        for (const char *ext: HeaderExtensions) {
            ...
            if (TestFile(header_filename, FileType::File)) {
                ...
                const char *text = Fmt(&str_alloc, StdErr->IsVt100(), "Run MOC on %!..+%1%!0 %!D..@%2%!0", header_filename, src.target->ns).ptr;
    ...
    if (moc_filename) {
        ...
        if (!obj_filename) {
            ...
            const char *text = Fmt(&str_alloc, StdErr->IsVt100(), "Build MOC for %!..+%1%!0 %!D..@%2%!0", src.filename, src.target->ns).ptr;
...
R"(#include <QtCore/QtPlugin>
    ...
    const char *src_filename = Fmt(&str_alloc, "%1%/%2_qt.cc", gen_directory, target.name).ptr;
