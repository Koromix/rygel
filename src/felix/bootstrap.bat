...
if NOT ERRORLEVEL 1 (
    ...
    if NOT ERRORLEVEL 1 (
        ...
        %BUILD%\felix.exe --no_presets --optimize=Fast --features=StaticLink -O %BUILD%\Fast felix
...
if NOT ERRORLEVEL 1 (
    ...
    mkdir %BUILD%
    g++ -std=gnu++2a -O0 -DNDEBUG -DNOMINMAX  -DWINVER=0x0601 -D_WIN32_WINNT=0x0601 -DUNICODE -D_UNICODE -DLIBCC_NO_BROTLI %SRC% -lws2_32 -w -o%BUILD%\felix.exe
    %BUILD%\felix.exe --no_presets --optimize=Fast --features=StaticLink -O %BUILD%\Fast felix
    move %BUILD%\Fast\felix.exe %BIN%
