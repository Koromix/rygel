...
bool Builder::AddTarget(const TargetInfo &target)
{
    ...
    if (target.type == TargetType::Executable) {
        ...
        if (!UpdateVersionSource(target, src_filename))
            return false;
...
bool Builder::UpdateVersionSource(const TargetInfo &target, const char *dest_filename)
{
    ...
    // Best effort!
    BuildGitVersion(target.version_tag, version_str);
...
bool Builder::BuildGitVersion(Span<const char> tag_name, Span<char> out_version)
{
    if (!versioneer.IsValid() && !versioneer.Prepare("."))
        return false;

    const char *str = versioneer.Version(tag_name.ptr);
    if (!str)
        return false;

    CopyString(str, out_version);
    return true;
}
