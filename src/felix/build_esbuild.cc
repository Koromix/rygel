...
bool Builder::PrepareEsbuild()
{
    ...
    {
        tsconfig_filename = Fmt(&str_alloc, "%1%/tsconfig.json", misc_directory).ptr;
...
const char *Builder::AddEsbuildSource(const SourceFileInfo &src)
{
    ...
    const char *meta_filename = build_map.FindValue({ src.target->ns, src.filename }, nullptr);
    ...
    // Build web bundle
    if (!meta_filename) {
        const char *bundle_filename = BuildObjectPath(src.target->ns, src.filename, cache_directory, "", "");
