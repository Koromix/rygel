...
async function test() {
    ...
    {
        let lib_path = path.resolve(__dirname, cnoke.output, 'win32.dll');
        let koffi_path = path.resolve(__dirname, '..');

        let child = spawnSync(process.execPath, ['-e', `
            const koffi = require(${JSON.stringify(koffi_path)});
            const lib = koffi.load(${JSON.stringify(lib_path)});
            lib.func('void SetCrashFilter()')();
            lib.func('void CrashNow()')();
        `], { timeout: 10000 });

        let stderr = child.stderr.toString();
        assert.ok(stderr.includes('FILTER_CALLED'),
            'UnhandledExceptionFilter was not called on crash');
